...
export function Sidebar() {
  ...
  const [editingArchive, setEditingArchive] = useState<ArchiveItem | null>(
    null
  );
  const [newFileName, setNewFileName] = useState("");
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [archiveToDelete, setArchiveToDelete] = useState<ArchiveItem | null>(
    null
  );
  ...
  const fetchArchives = async (retryCount = 0) => {
    ...
    try {
      ...
      if (error.code === "ECONNABORTED" || error.message?.includes("timeout")) {
        if (retryCount < 2) {
          console.log(
            `Retrying sidebar archive fetch (attempt ${retryCount + 1})...`
          );
          setTimeout(() => {
            fetchArchives(retryCount + 1);
  ...
  const handleRenameSubmit = async () => {
    ...
    try {
      ...
      const response = await archiveAPI.updateArchive(editingArchive.id, {
    ...
    >
                  ...
                  >
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent
                          side="top"
                          className="text-[10px] px-2 py-1 bg-white border border-gray-200"
                        >
                          {(() => {
                            try {
        ...
        <DialogContent className="sm:max-w-[425px] z-[9999] bg-white shadow-2xl border-2 border-gray-300">
          ...
          <DialogFooter className="flex gap-2">
            ...
            <Button
              ...
              disabled={
                !newFileName.trim() ||
                newFileName.trim() === editingArchive?.file_name
              }
        ...
        <AlertDialogContent className="sm:max-w-[425px] bg-white shadow-2xl border-2 border-gray-300">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-gray-600">
              Are you sure you want to delete "{archiveToDelete?.file_name}"?
      ...
      <div className="space-y-1 pt-3 border-sidebar-border">
        ...
        {menuOpen && (
          <div
            ref={menuRef}
            className={`absolute bottom-20 left-0 w-48 bg-white border border-gray-200 rounded-md shadow-sm z-50 animate-fade-in ${
              collapsed ? "justify-center ml-0 w-auto" : "ml-12"
            }`}
          >
            {/* Settings */}
            <TooltipProvider>
            ...
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button
                  ...
                  } gap-3 text-red-600 hover:bg-red-50`}
                  onClick={(e) => {
                    e.stopPropagation();
                ...
                >
                  ...
                  <span
                    className={`${collapsed ? "hidden" : "inline"} text-xs`}
                  >
                    Logout
                  </span>
                </Button>
